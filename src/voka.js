...
function Voka(options = {}) {
    ...
    this.opt = Object.assign(
        {
            ...
            cssClass: [],
            footer: false,
            closeMethod: ["button", "overlay", "esc"],
        },
        options,
    );
    ...
    this._allowBackdropMethod = closeMethod.includes("overlay");
    this._footerButtons = [];
    this._handlerEsc = this._handlerEsc.bind(this);
}

Voka.prototype.getScrollWidth = function () {
    if (Voka.scrollWidth) return Voka.scrollWidth;
    ...
    Voka.scrollWidth = div.offsetWidth - div.clientWidth;
    document.body.removeChild(div);
    return Voka.scrollWidth;
};
...
Voka.prototype.build = function () {
    ...
    if (this._allowButtonMethod) {
        const closeBtn = document.createElement("button");
        closeBtn.className = "voka__close";
    ...
    if (this.opt.footer) {
        ...
        if (this._footerContent) {
            this._footerElement.innerHTML = this._footerContent;
        }
        if (this._footerButtons.length > 0) {
            this._footerButtons.forEach((button) => {
                this._footerElement.append(button);
            });
        }
        container.appendChild(this._footerElement);
    }
    this._backdrop.appendChild(container);
    if (this._allowBackdropMethod) {
...
Voka.prototype._toggleBodyScroll = function (isDisable) {
    if (isDisable) {
...
Voka.prototype.open = function () {
    if (this._backdrop?.classList.contains("voka--show")) return this._backdrop;
    if (!this._backdrop) {
        this.build();
    }
...
Voka.prototype._handlerEsc = function (e) {
    const lastModal = Voka.element[Voka.element.length - 1];
...
Voka.prototype.close = function (destroy = this.opt.destroyOnClose) {
    if (!this._backdrop) return;
    if (!this._backdrop.classList.contains("voka--show")) return;
    this._backdrop.classList.remove("voka--show");
...
Voka.prototype.destroy = function () {
    this.close(true);
};
